...
namespace symbolic {
UnorderedSelector::UnorderedSelector(const plugins::Options &opts)
    : PlanSelector(opts) {
...
void UnorderedSelector::save_accepted_plan(
    ...
    if (num_accepted_plans == 0) {
        ...
        if (!write_plans) {
            plan_mgr.save_plan(
                ordered_plan, plan_mgr_task_proxy, false,
                num_desired_plans > 1);
        }
    }
    ...
    if (write_plans) {
        plan_mgr.save_plan(
            ordered_plan, plan_mgr_task_proxy, dump_plans,
            num_desired_plans > 1);
    }
}

class UnorderedSelectorFeature
    : public plugins::TypedFeature<PlanSelector, UnorderedSelector> {
public:
