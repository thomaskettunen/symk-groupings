...
public:
    ...
    int filterMutexBucket(
        std::vector<BDD> &bucket, bool fw, bool initialization, int max_time,
        int max_nodes);

    void print_symbolic_task_size() const;

    void set_time_limit(int maxTime) {
        sym_vars->set_time_limit(maxTime);
    }
    ...
    std::shared_ptr<SymTransitionRelations> get_transition_relations() const {
        return std::make_shared<SymTransitionRelations>(
            sym_transition_relations);
    }
};
}
#endif
